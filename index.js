...
btn.addEventListener(`click`, () => {
    if(input.value > 14 || input.value < 1) {
        input.value = ``;
        alert (`Ты чё читать не умеешь?`);
    } 
    let num = input.value;
    input.value = ``;
    const krasota = readAndSolve("ошибка", `${arr[num]}`);
    const div1 = document.createElement(`div`);
    

    // for(let i = 0; i < krasota.length; i++) {
    //     if(i === 82) {
    //         break;
    //     }
    //     let str = krasota[i]
    //     const span = document.createElement(`div`)
    //     span.className = `Slova`
    //     span.innerText = str;
    //     div1.append(span)
    //     if(i % 9 === 0) {
    //         const br = '<br>'
