function solve(boardString) {
    ...
    // console.table(intResult)
    return intResult;
  }
  
  function getRandom(arr) {
    const memoryArr = JSON.parse(JSON.stringify((arr)));
    for (let i = 0; i < 9; i++) {
      for (let j = 0; j < 9; j++) {
        if (arr[i][j] === '-') arr[i][j] = Math.floor(Math.random() * 9 + 1);
      }
    } if(isSolved2(arr)) return arr;
    getRandom(memoryArr);
  }
      ...
      for (j; j < 9; j++) {
        ...
        if (result[i][j] === '-') {
          ...
          for (let b = 0; b < base.length; b++) {
            if (!summ.includes(base[b]) && isSolved2(result)) {
              result[i][j] = base[b];
              return result;
            }
          }
